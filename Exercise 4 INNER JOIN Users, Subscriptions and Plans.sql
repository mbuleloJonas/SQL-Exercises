...
SELECT A.user_name, A.country, C.plan_name, C.monthly_price, B.start_date FROM the.users.table AS A INNER JOIN the.subscriptions.table AS B ON A.user_id = B.user_id INNER JOIN the.plans.table AS C ON B.plan_id = C.plan_id;
