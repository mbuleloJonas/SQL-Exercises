-- Exercise 4: Show every plan and every subscription, including plans without subscribers AND any subscription referencing a plan that does not exist.
...
SELECT A.plan_id, A.plan_name, B.susbcription_id, B.user_id FROM the.plans.table AS A FULL OUTER JOIN the.subscriptions.table AS B ON A.plan_id = B.plan_id;
